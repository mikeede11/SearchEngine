...
public abstract class MinHeap<E extends Comparable> {
    ...
    protected boolean bTreeSet = false;
    ...
    public MinHeap(){
        this.elements = (E[]) new Comparable[2];// TODO: 4/21/2020 check with proffessor.
    ...
    protected  boolean isGreater(int i, int j) {
        if(bTreeSet){
            return bTree.get((URI)elements[i]).compareTo(bTree.get((URI) this.elements[j])) > 0;
        }
        return this.elements[i].compareTo(this.elements[j]) > 0;
    }//this might need to change to bTree.get
